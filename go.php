...
if(!isset($_GET['code'])&&isset($_GET['scope'])){
	...
	\DataCenter\SystemTool::systemLog($db,'网页授权开始','scope',$_SERVER['QUERY_STRING']);
	\LaneWeChat\Core\WeChatOAuth::getCode($redirect_uri, $state=1, $scope);
}elseif(isset($_GET['code'])){
	$code = $_GET['code'];
	if(empty($code)){
		\DataCenter\SystemTool::systemLog($db,'code回调','获取code异常，code为空','1');
		echo '网页授权错误，获取code异常';
		exit;
	}
	\DataCenter\SystemTool::systemLog($db,'网页授权得到code','code',$_SERVER['QUERY_STRING']);
	...
	if(empty($tempinfo['openid'])){
		...
		\DataCenter\SystemTool::systemLog($db,'网页授权','获取access_token异常，没有取到openid',$extradata);
	...
	//插入用户信息
	if(!\DataCenter\Userinfo::checkIfuserInfoinDb($db,$tempinfo['openid'])){
		...
		if(!empty($userinfo)&&!isset($userinfo['errcode'])){
			...
			\DataCenter\SystemTool::systemLog($db,'网页授权','获取用户信息异常',json_encode($userinfo));
		}
	}
	$isSubscribe = \LaneWeChat\Core\UserManage::checkisSubscribe($tempinfo['openid']);
	$subscribe = $isSubscribe?1:0;
	\DataCenter\Userinfo::updateSubscribe($db,$tempinfo['openid'],$subscribe);
